...
// URL base para Edge Functions (search-obras, etc.)
...
// ── Búsqueda full-text (Edge Function search-obras) ───────────────────────────
/**
 * Busca obras mediante full-text search usando el Edge Function search-obras.
 *
 * Requiere que la migración 20260628_add_fulltext_obras.sql esté aplicada
 * (columna search_vector tsvector + índice GIN en tabla obras).
 *
 * El Edge Function usa plainto_tsquery('spanish', q) sobre search_vector,
 * lo que permite buscar por título, artista y descripción con normalización
 * de lemas en español (tildes, plurales, conjugaciones).
 *
 * @param {string} q - Término de búsqueda
 * @param {Object} [filters] - Filtros adicionales: { technique, year, tags[] }
 * @param {number} [limit=50] - Máximo de resultados (tope 100)
 * @returns {Promise<{ data: Array, total: number, error: any }>}
 */
export async function searchObras(q, filters = {}, limit = 50) {
  if (!q || !q.trim()) return { data: [], total: 0, error: null };

  const url = new URL(`${FUNCTIONS_URL}/search-obras`);
  url.searchParams.set('q', q.trim());
  if (filters.technique) url.searchParams.set('tecnica', filters.technique);
  if (filters.year) {
    // Año exacto → year_min = year_max
    url.searchParams.set('year_min', String(filters.year));
    url.searchParams.set('year_max', String(filters.year));
  }
  (filters.tags ?? []).forEach((tagId) => url.searchParams.append('tags', tagId));
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(`search-obras ${res.status}: ${body}`);
    }

    const { data, total } = await res.json();
    return { data: data ?? [], total: total ?? (data?.length ?? 0), error: null };
  } catch (err) {
    console.error('❌ Error en searchObras:', err);
    return { data: [], total: 0, error: err };
  }
}
