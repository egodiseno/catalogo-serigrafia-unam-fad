...
class PortafolioManager {
  ...
  _renderTablaObras(obras) {
    ...
    const estadoBadge = est => {
      const map = { publicado: 'badge-publicado', borrador: 'badge-borrador', revision: 'badge-revision' };
      const cls   = map[est] || 'badge-borrador';
      const label = est ? est.charAt(0).toUpperCase() + est.slice(1) : '—';
