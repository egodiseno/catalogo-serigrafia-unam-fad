/*
  ...
  Entrega 7 — Vista por técnica pública.
  Scope: renderiza una vista representativa para Serigrafía experimental a partir del dataset mock aprobado, incluyendo resumen documental y tags frecuentes.
  No implementa nuevas fichas, visor adicional, compartir, vista por serie, backend, admin ni ecommerce.
*/

(function () {
  const app = document.querySelector('[data-technique-app]');
  if (!app) return;

  const DATASET_URL = 'data/artworks.json';
  const techniqueSlug = app.dataset.techniqueSlug || '';
  const techniqueName = app.dataset.techniqueName || 'Técnica seleccionada';

  const elements = {
    count: app.querySelector('[data-technique-count]'),
    years: app.querySelector('[data-technique-years]'),
    series: app.querySelector('[data-technique-series]'),
    tags: app.querySelector('[data-technique-tags]'),
    progress: app.querySelector('[data-technique-progress]'),
    loading: app.querySelector('[data-technique-loading]'),
    error: app.querySelector('[data-technique-error]'),
    errorDetail: app.querySelector('[data-technique-error-detail]'),
    empty: app.querySelector('[data-technique-empty]'),
    grid: app.querySelector('[data-technique-grid]')
  };
  ...
  const mapArtwork = (artwork, index) => ({
    ...
    techniqueSlug: getArtworkField(artwork, ['techniqueSlug', 'technique_slug', 'slugTecnica'], ''),
  ...
  const matchesTechnique = (artwork) => {
    if (techniqueSlug && artwork.techniqueSlug === techniqueSlug) return true;
    return normalizeText(artwork.technique) === normalizeText(techniqueName);
  };
  ...
  const updateSummary = (artworks) => {
    ...
    const series = unique(artworks.map((artwork) => artwork.series).filter((value) => normalizeText(value) !== 'sin serie'));
    ...
    if (elements.series) elements.series.textContent = series.length > 0 ? series.join(' · ') : 'Sin series documentadas';
    if (elements.tags) elements.tags.textContent = frequentTags.length > 0 ? frequentTags.join(' · ') : 'Sin tags registrados';
    if (elements.progress) {
      elements.progress.textContent = `${artworks.length} ${artworks.length === 1 ? 'obra asociada' : 'obras asociadas'} a ${techniqueName}`;
    }
  };
  ...
  const initialize = async () => {
    ...
    try {
      ...
      const artworks = Array.isArray(data) ? data.filter(isPublished).map(mapArtwork).filter(matchesTechnique) : [];
      ...
      setStatus('error');
      if (elements.progress) elements.progress.textContent = 'No se pudo preparar la vista por técnica';
